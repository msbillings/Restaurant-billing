...
async function run() {
    ...
    for (let i = 4; i <= 10; i++) {
        ...
        markdownOutput += `## Restaurant ${i}: ${res.restaurantName}\n\n`;
        markdownOutput += `**SuperAdmin Email:** ${res.email} | **Master Password:** ${res.password}\n`;
        markdownOutput += `**License Key:** ${res.licenseKey}\n\n`;
        markdownOutput += `| Field | Login | Password | Notes |\n`;
        markdownOutput += `|---|---|---|---|\n`;
        markdownOutput += `| Owner/Admin | admin${i} | ${res.password} | |\n`;
        markdownOutput += `| Cashier | cashier${i} | CashierPass@${i} | |\n`;
        markdownOutput += `| Captain | captain${i} | CaptainPass@${i} | |\n\n`;
    }
    
    fs.writeFileSync('7_testing_credentials.md', markdownOutput);
    fs.writeFileSync('7_testing_credentials.json', JSON.stringify(results, null, 2));
    console.log("Done!");
    process.exit(0);
}
