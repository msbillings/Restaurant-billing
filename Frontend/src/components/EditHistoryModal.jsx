...
const EditHistoryModal = ({ order, onClose }) => {const { t } = useLanguage();
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-surface w-full max-w-4xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden border border-border">
        ...
        <div className="p-5 border-b border-border flex justify-between items-center bg-background">
          <div>
            <h2 className="text-xl font-bold text-text-main flex items-center gap-2">
              <FileSignature className="text-primary" size={24} />{t("Audit Trail:")}
              {order.billNumber || order.tableNo}
            </h2>
            <p className="text-sm text-text-muted mt-1">{t("Detailed history of all modifications to this order")}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-5 bg-surface space-y-6">
          ...
          <div className="text-center text-text-muted py-10">{t("No edit history recorded for this bill.")}</div> :
          ...
          <div key={idx} className="border border-border rounded-xl overflow-hidden shadow-sm">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-border">
                  ...
                  <div className="p-4 bg-red-50/10">
                    <h4 className="text-xs font-bold text-text-muted uppercase tracking-wider mb-3 text-red-500">{t("Before Edit")}</h4>
                  ...
                  <div className="text-sm text-text-muted italic">{t("No items")}</div>
                  }
                    </div>
                  ...
                  <div className="p-4 bg-green-50/10 relative">
                    ...
                    <h4 className="text-xs font-bold text-text-muted uppercase tracking-wider mb-3 text-green-500">{t("After Edit")}</h4>
                    <div className="space-y-2 mb-4">
