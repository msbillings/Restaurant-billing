...
const DeliveryOrders = ({ onNavigate, onGoBack }) => {
  const { t } = useLanguage();
  ...
  useEffect(() => {
    fetchDeliveryOrders();
  }, [currentPage, searchTerm, platformFilter, startDate, endDate]);
  ...
  const getPlatformColor = (platform) => {
    switch (platform) {
      case 'Swiggy': return 'bg-orange-100 text-orange-800 dark:bg-orange-950/50 dark:text-orange-300';
      case 'Zomato': return 'bg-red-100 text-red-800 dark:bg-red-950/50 dark:text-red-300';
      case 'Direct': return 'bg-blue-100 text-blue-800 dark:bg-blue-950/50 dark:text-blue-300';
      case 'Other': return 'bg-gray-100 text-gray-800 dark:bg-zinc-800 dark:text-zinc-300';
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-2 sm:gap-3 mb-2 sm:mb-2.5 p-2 sm:p-2.5 bg-surface rounded-2xl border border-border shadow-xs shrink-0">
        <div className="flex items-center gap-3 shrink-0">
          ...
          <div>
            ...
            <p className="text-[11px] sm:text-xs text-text-muted font-medium">{t("View and manage delivery orders")}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap lg:flex-nowrap items-center gap-2 sm:gap-2.5 w-full lg:w-auto lg:flex-1 lg:justify-end">
          ...
          <div className="relative flex-1 min-w-[170px] max-w-full">
            ...
            <input
              type="text"
              placeholder={t("Search Bill #, Delivery #, Customer...")}
              value={searchTerm}
              onChange={(e) => {
          ...
          {/* Dynamic Delivery Orders Count Badge */}
          <div className="inline-flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 rounded-xl bg-orange-50 text-orange-800 border border-orange-200 text-xs font-bold shrink-0 shadow-2xs">
            ...
            <span>{pagination.totalBills || 0} {t("Delivery Orders")}</span>
          </div>
        ...
        <div className="hidden md:block overflow-y-auto flex-1 custom-scrollbar">
          <table className="w-full text-left border-collapse">
            <thead className="bg-background/80 sticky top-0 z-10 backdrop-blur-xs">
              <tr>
                <th className="p-3.5 font-bold text-text-muted border-b border-border text-xs uppercase tracking-wider">{t("Delivery #")}</th>
                ...
                <th className="p-3.5 font-bold text-text-muted border-b border-border text-xs uppercase tracking-wider">{t("Platform")}</th>
              ...
              {loading ? (
                [...Array(6)].map((_, i) => (
                  <tr key={i} className="border-b border-border animate-pulse">
                    ...
                    <td className="p-3.5"><div className="w-16 h-5 bg-background rounded-full"></div></td>
                    <td className="p-3.5"><div className="w-14 h-5 bg-background rounded-full"></div></td>
                    <td className="p-3.5"><div className="w-18 h-4 bg-background rounded"></div></td>
                ...
                filteredOrders.map((order) => (
                  <tr key={order._id} className="border-b border-border hover:bg-surface-hover/70 transition-colors">
                    {/* Delivery # Badge */}
                    <td className="p-3.5">
                      <span className="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-mono font-black bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-900/50">
                        {order.tableNo && order.tableNo.startsWith('DEL-') ? order.tableNo : (order.tableNo || 'DEL')}
                      </span>
                    </td>
                    {/* Bill # */}
                ...
                <div className="flex justify-between items-start">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center px-2 py-0.5 rounded-lg text-xs font-mono font-black bg-orange-50 dark:bg-orange-950/40 text-orange-600 dark:text-orange-400 border border-orange-200 dark:border-orange-900/50">
                        {order.tableNo && order.tableNo.startsWith('DEL-') ? order.tableNo : (order.tableNo || 'DEL')}
                      </span>
