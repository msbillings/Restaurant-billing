...
import { Search, ChevronLeft, ChevronRight, Star, X, ChevronDown, Image as ImageIcon, 
  ...
  Users, Smile, Soup, Popcorn, Scroll, Beef, Cookie
} from 'lucide-react';
...
const MenuGrid = ({ onSelectItem, searchTerm = '', onSearchChange, isLayoutLocked = false }) => {
  ...
  return (
    <div className="flex flex-row h-full bg-white overflow-hidden">
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 flex-1 min-w-0 pr-1">
                  {getCategoryIcon(cat)}
                  <span className="break-words leading-tight whitespace-pre-wrap font-medium">{cat === '⭐ Favourites' ? 'Favorite Items' : cat.replace('⭐ ', '')}</span>
                </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
        ...
        {/* Search Bar Row */}
        <div className="flex items-center bg-gray-50 border-b border-gray-200 h-[60px] shrink-0 px-3 gap-3">
          <div className="flex-1 flex items-center h-[42px] px-4 bg-white rounded-xl border border-gray-200 shadow-sm focus-within:border-red-500 transition-all">
          ...
          <div className="w-[120px] flex items-center h-[42px] px-3 bg-white rounded-xl border border-gray-200 shadow-sm focus-within:border-red-500 transition-all hidden sm:flex">
            <input 
              type="text"
          ...
          <button
            ...
            className={`flex items-center justify-center h-[42px] px-3 sm:px-4 rounded-xl border shadow-sm transition-all gap-2 font-bold text-[13px] ${showImages ? 'bg-red-50 text-red-600 border-red-200' : 'bg-white text-gray-500 border-gray-200 hover:bg-gray-50'}`}
          >
            <ImageIcon size={16} />
        ...
        <div className="flex-1 overflow-y-auto p-4 relative bg-gray-50 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
          ...
          {filteredItems.map(item => {
            ...
            <div
              ...
              } ${showImages ? 'min-h-[170px]' : 'h-[120px] p-3'}`}
              onClick={() => {
            ...
            >
              <div className={`flex items-start justify-between w-full h-4 z-10 absolute ${showImages ? 'top-2 left-0 px-2' : 'top-3 left-0 px-3'}`}>
                ...
                <div className="flex gap-1.5 items-center">
                  {!isAvailable && <span className={`text-[9px] font-bold uppercase px-1.5 py-0.5 rounded-md shadow-sm ${showImages ? 'text-white bg-red-500/90 backdrop-blur-sm' : 'text-red-500 bg-red-50'}`}>Out of Stock</span>}
              ...
              {showImages && item.image ? (
                <div className="w-full h-[90px] shrink-0 bg-gray-100 relative">
                  <img src={formatImageUrl(item.image)} alt={item.name} className="w-full h-full object-cover" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/40 via-transparent to-transparent"></div>
                </div>
              ) : (showImages && (
                <div className="w-full h-[90px] bg-gray-50 flex flex-col items-center justify-center shrink-0 text-gray-400 border-b border-gray-100 border-dashed">
