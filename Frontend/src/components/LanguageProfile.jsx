...
const LanguageProfile = ({ onNavigate }) => {
    ...
    <div className="h-full flex flex-col bg-gray-50 p-6 overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-800 flex items-center gap-2">
              <Languages className="text-primary" /> Language Profiles
            </h1>
            <p className="text-sm text-gray-500">Set the default interface language for your personal account</p>
          </div>
        </div>
        <button 
          onClick={handleSave}
      ...
      <div className="flex-1 max-w-3xl mx-auto w-full bg-white rounded-xl shadow-sm border border-gray-200 p-8 flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-1/3 flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-gray-100 pb-8 md:pb-0 pr-0 md:pr-8">
          ...
          <h2 className="text-xl font-bold text-gray-800">Current User</h2>
          <p className="text-gray-500 text-sm">Personal Interface Settings</p>
          <div className="mt-6 w-full p-4 bg-primary/10 rounded-lg text-center">
            <span className="block text-xs font-bold text-primary uppercase mb-1">Active Profile Language</span>
            <span className="font-bold text-gray-800">{languages.find(l => l.code === selectedLang)?.name || 'English'}</span>
          </div>
        </div>

        <div className="w-full md:w-2/3">
          <h3 className="text-lg font-bold text-gray-800 mb-6">Select your preferred language</h3>
